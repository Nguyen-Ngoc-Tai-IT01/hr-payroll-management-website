...
const PayrollList = () => {
  ...
  useEffect(() => {
    fetch("http://localhost:5000/api/payroll")
      ...
      .catch((err) => {
        ...
        setPayrolls([]);
        setErrorMsg(
          "Không thể kết nối đến Backend hoặc Backend đang bị lỗi. Vui lòng kiểm tra Terminal của Backend.",
        );
        setLoading(false);
      });
  }, []);
  ...
  const filteredPayrolls = useMemo(() => {
    ...
    return payrolls.filter((row) => {
      ...
      const matchSearch =
        safeEmployeeId.includes(searchLower) || safeName.includes(searchLower);
            ...
            {filteredPayrolls.length > 0 ? (
              ...
              <tr>
                <td colSpan="8" className={styles.noData}>
                  {errorMsg
                    ? "Không có dữ liệu do lỗi Backend."
